...
namespace SupplementLand.Infrastructure.Services
{
    public class FactoryService : IFactoryService
    {
        ...
        public async Task<OperationResult> AddFactoryImage(FacImageDto dto)
        {
            var existingFac=await GetFactoryById(dto.FactoryId);
            if (existingFac == null) { return new OperationResult { Success = false, Message = "This factory doesn't exist" }; }
            existingFac.Id=dto.FactoryId;
            existingFac.ImageUrl=dto.ImageUrl;
            await _context.SaveChangesAsync();
            return new OperationResult { Success = true, Message = "Image added successfully" };
